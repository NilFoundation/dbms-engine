...
// kMinOverlappingRatio first when you tune your database.
        enum compaction_pri : char {
            ...
            // Try this if you only update some hot keys in small ranges.
                    kOldestLargestSeqFirst = 0x1, // First compact files whose range hasn't been compacted to the next level
            // for the longest. If your updates are random across the key space,
            // write amplification is slightly better with this option.
                    kOldestSmallestSeqFirst = 0x2, // First compact files whose ratio between overlapping size in next level
        ...
        struct compaction_options_fifo {
            ...
            compaction_options_fifo(uint64_t _max_table_files_size, bool _allow_compaction) : max_table_files_size(
...
// Compression opts for different compression algorithms like Zlib
        struct compression_options {
            ...
            compression_options() : window_bits(-14), level(kDefaultCompressionLevel), strategy(0), max_dict_bytes(0),
                    zstd_max_train_bytes(0), enabled(false) {
            }

            compression_options(int wbits, int _lev, int _strategy, int _max_dict_bytes, int _zstd_max_train_bytes,
                                bool _enabled) : window_bits(wbits), level(_lev), strategy(_strategy),
                    max_dict_bytes(_max_dict_bytes), zstd_max_train_bytes(_zstd_max_train_bytes), enabled(_enabled) {
            }
        };

        enum update_status {    // Return status For inplace update callback
        ...
        struct advanced_column_family_options {
            ...
            std::vector<compression_type> compression_per_level;
            ...
            table_properties_collector_factories_type table_properties_collector_factories;
