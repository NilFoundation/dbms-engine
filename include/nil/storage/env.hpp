...
// Options while opening a file to read/write
        ...
        public:
            ...
            static environment_type *Default();
            ...
            virtual status_type NewSequentialFile(const std::string &fname, std::unique_ptr<SequentialFile> *result,
            ...
            //
            // The returned file will only be accessed by one thread at a time.
            virtual status_type ReopenWritableFile(const std::string & /*fname*/,
            ...
            virtual status_type ReuseWritableFile(const std::string &fname, const std::string &old_fname,
                                                  std::unique_ptr<writable_file> *result,
                                                  const environment_options &options);
            ...
            //
            // The returned file will only be accessed by one thread at a time.
            virtual status_type NewRandomRWFile(const std::string & /*fname*/,
                                                std::unique_ptr<random_rw_file> * /*result*/,
                                                const environment_options & /*options*/) {
            ...
            virtual status_type NewMemoryMappedFileBuffer(const std::string & /*fname*/,
            ...
            // *result and returns OK. On failure stores nullptr in *result and
            // returns non-OK.
            virtual status_type NewDirectory(const std::string &name, std::unique_ptr<Directory> *result) = 0;
            ...
            virtual status_type FileExists(const std::string &fname) = 0;
            ...
            //         IOError if an IO Error was encountered
            virtual status_type GetChildren(const std::string &dir, std::vector<std::string> *result) = 0;
            ...
            //                  permission to access "dir", or if "dir" is invalid.
            //         IOError if an IO Error was encountered
            virtual status_type GetChildrenFileAttributes(const std::string &dir, std::vector<file_attributes> *result);

            // Delete the named file.
            virtual status_type DeleteFile(const std::string &fname) = 0;
            ...
            virtual status_type CreateDir(const std::string &dirname) = 0;

            // Creates directory if missing. Return Ok if it exists, or successful in
            // Creating.
            virtual status_type CreateDirIfMissing(const std::string &dirname) = 0;

            // Delete the specified directory.
            virtual status_type DeleteDir(const std::string &dirname) = 0;

            // Store the size of fname in *file_size.
            virtual status_type GetFileSize(const std::string &fname, uint64_t *file_size) = 0;

            // Store the last modification time of fname in *file_mtime.
            virtual status_type GetFileModificationTime(const std::string &fname, uint64_t *file_mtime) = 0;

            // Rename file src to target.
            virtual status_type RenameFile(const std::string &src, const std::string &target) = 0;

            // Hard Link file src to target.
            virtual status_type LinkFile(const std::string & /*src*/, const std::string & /*target*/) {
                return status_type::NotSupported("LinkFile is not supported for this environment_type");
            }

            virtual status_type NumFileLinks(const std::string & /*fname*/, uint64_t * /*count*/) {
                return status_type::NotSupported(
                        "Getting number of file links is not supported for this environment_type");
            }

            virtual status_type AreFilesSame(const std::string & /*first*/, const std::string & /*second*/,
                                             bool * /*res*/) {
                return status_type::NotSupported("AreFilesSame is not supported for this environment_type");
            }
            ...
            virtual status_type LockFile(const std::string &fname, FileLock **lock) = 0;

            // release the lock acquired by a previous successful call to LockFile.
            // REQUIRES: lock was returned by a successful LockFile() call
            // REQUIRES: lock has not already been unlocked.
            virtual status_type UnlockFile(FileLock *lock) = 0;
            ...
            // same directory.
            virtual status_type GetTestDirectory(std::string *path) = 0;

            // Create and return a log file for storing informational messages.
            virtual status_type NewLogger(const std::string &fname, std::shared_ptr<Logger> *result) = 0;
            ...
            virtual status_type GetHostName(char *name, uint64_t len) = 0;

            // Get the number of seconds since the Epoch, 1970-01-01 00:00:00 (UTC).
            // Only overwrites *unix_time on success.
            virtual status_type GetCurrentTime(int64_t *unix_time) = 0;

            // Get full directory name for this db.
            virtual status_type GetAbsolutePath(const std::string &db_path, std::string *output_path) = 0;
            ...
            virtual status_type SetAllowNonOwnerAccess(bool /*allow_non_owner_access*/) {
            ...
            // OptimizeForLogWrite will create a new environment_options object that is a copy of
            // the environment_options in the parameters, but is optimized for reading log files.
            virtual environment_options OptimizeForLogRead(const environment_options &env_options) const;

            // OptimizeForManifestRead will create a new environment_options object that is a copy
            // of the environment_options in the parameters, but is optimized for reading manifest
            // files.
            virtual environment_options OptimizeForManifestRead(const environment_options &env_options) const;
            ...
            virtual environment_options OptimizeForManifestWrite(const environment_options &env_options) const;

            // OptimizeForCompactionTableWrite will create a new environment_options object that is
...
#undef GetFreeSpace
            ...
            virtual status_type GetFreeSpace(const std::string & /*path*/, uint64_t * /*diskfree*/) {
                return status_type::NotSupported();
            }
...
// The factory function to construct a ThreadStatusUpdater.  Any environment_type
...
// A file abstraction for reading sequentially through a file
        ...
        public:
            ...
            virtual status_type InvalidateCache(size_t /*offset*/, size_t /*length*/) {
                return status_type::NotSupported("InvalidateCache not supported.");
            }

            // Positioned Read for direct I/O
            // If Direct I/O enabled, offset, n, and scratch should be properly aligned
            virtual status_type PositionedRead(uint64_t /*offset*/, size_t /*n*/, slice * /*result*/,
                                               char * /*scratch*/) {
                return status_type::NotSupported();
            }
        };
...
// at a time to the file.
        ...
        public:
            ...
            virtual status_type PositionedAppend(const slice & /* data */, uint64_t /* offset */) {
                return status_type::NotSupported();
            }
             ...
             */
            virtual void SetIOPriority(environment_type::IOPriority pri) {
                io_priority_ = pri;
            }

            virtual environment_type::IOPriority GetIOPriority() {
                return io_priority_;
            }

            virtual void SetWriteLifeTimeHint(environment_type::write_life_time_hint hint) {
                write_hint_ = hint;
            }

            virtual environment_type::write_life_time_hint GetWriteLifeTimeHint() {
                return write_hint_;
            }
            ...
            virtual void GetPreallocationStatus(size_t *block_size, size_t *last_allocated_block) {
            ...
            virtual status_type RangeSync(uint64_t /*offset*/, uint64_t /*nbytes*/) {
                return status_type();
            }
        ...
        protected:
            ...
            environment_type::IOPriority io_priority_;
...
// An interface for writing log messages.
        ...
        public:
            ...
            explicit Logger(const InfoLogLevel log_level = InfoLogLevel::INFO_LEVEL) : closed_(false),
...
// Identifies a locked file.
        ...
        extern void Log(const InfoLogLevel log_level, const std::shared_ptr<Logger> &info_log, const char *format, ...);
...
#   endif
        ...
        extern void Log(const InfoLogLevel log_level, Logger *info_log, const char *format, ...);
...
// functionality of another environment_type.
        ...
        public:
            ...
            status_type NewSequentialFile(const std::string &f, std::unique_ptr<SequentialFile> *r,
            ...
            status_type NewWritableFile(const std::string &f, std::unique_ptr<writable_file> *r,
                                        const environment_options &options) override {
                return target_->NewWritableFile(f, r, options);
            }

            status_type ReopenWritableFile(const std::string &fname, std::unique_ptr<writable_file> *result,
                                           const environment_options &options) override {
                return target_->ReopenWritableFile(fname, result, options);
            }

            status_type ReuseWritableFile(const std::string &fname, const std::string &old_fname,
                                          std::unique_ptr<writable_file> *r,
                                          const environment_options &options) override {
                return target_->ReuseWritableFile(fname, old_fname, r, options);
            }

            status_type NewRandomRWFile(const std::string &fname, std::unique_ptr<random_rw_file> *result,
                                        const environment_options &options) override {
                return target_->NewRandomRWFile(fname, result, options);
            }

            status_type NewDirectory(const std::string &name, std::unique_ptr<Directory> *result) override {
                return target_->NewDirectory(name, result);
            }

            status_type FileExists(const std::string &f) override {
                return target_->FileExists(f);
            }

            status_type GetChildren(const std::string &dir, std::vector<std::string> *r) override {
                return target_->GetChildren(dir, r);
            }

            status_type GetChildrenFileAttributes(const std::string &dir,
                                                  std::vector<file_attributes> *result) override {
                return target_->GetChildrenFileAttributes(dir, result);
            }

            status_type DeleteFile(const std::string &f) override {
                return target_->DeleteFile(f);
            }

            status_type CreateDir(const std::string &d) override {
                return target_->CreateDir(d);
            }

            status_type CreateDirIfMissing(const std::string &d) override {
                return target_->CreateDirIfMissing(d);
            }

            status_type DeleteDir(const std::string &d) override {
                return target_->DeleteDir(d);
            }

            status_type GetFileSize(const std::string &f, uint64_t *s) override {
                return target_->GetFileSize(f, s);
            }

            status_type GetFileModificationTime(const std::string &fname, uint64_t *file_mtime) override {
                return target_->GetFileModificationTime(fname, file_mtime);
            }

            status_type RenameFile(const std::string &s, const std::string &t) override {
                return target_->RenameFile(s, t);
            }

            status_type LinkFile(const std::string &s, const std::string &t) override {
                return target_->LinkFile(s, t);
            }

            status_type NumFileLinks(const std::string &fname, uint64_t *count) override {
                return target_->NumFileLinks(fname, count);
            }

            status_type AreFilesSame(const std::string &first, const std::string &second, bool *res) override {
                return target_->AreFilesSame(first, second, res);
            }

            status_type LockFile(const std::string &f, FileLock **l) override {
                return target_->LockFile(f, l);
            }

            status_type UnlockFile(FileLock *l) override {
            ...
            status_type GetTestDirectory(std::string *path) override {
                return target_->GetTestDirectory(path);
            }

            status_type NewLogger(const std::string &fname, std::shared_ptr<Logger> *result) override {
            ...
            status_type GetHostName(char *name, uint64_t len) override {
                return target_->GetHostName(name, len);
            }

            status_type GetCurrentTime(int64_t *unix_time) override {
                return target_->GetCurrentTime(unix_time);
            }

            status_type GetAbsolutePath(const std::string &db_path, std::string *output_path) override {
            ...
            status_type SetAllowNonOwnerAccess(bool allow_non_owner_access) override {
            ...
            status_type GetThreadList(std::vector<ThreadStatus> *thread_list) override {
            ...
            environment_options OptimizeForLogRead(const environment_options &env_options) const override {
                return target_->OptimizeForLogRead(env_options);
            }

            environment_options OptimizeForManifestRead(const environment_options &env_options) const override {
            ...
            environment_options OptimizeForManifestWrite(const environment_options &env_options) const override {
...
// protected virtual methods.
        ...
        public:
            ...
            status_type PositionedAppend(const slice &data, uint64_t offset) override {
            ...
            void SetIOPriority(environment_type::IOPriority pri) override {
                target_->SetIOPriority(pri);
            }

            environment_type::IOPriority GetIOPriority() override {
                return target_->GetIOPriority();
            }

            void SetWriteLifeTimeHint(environment_type::write_life_time_hint hint) override {
                target_->SetWriteLifeTimeHint(hint);
            }

            environment_type::write_life_time_hint GetWriteLifeTimeHint() override {
            ...
            status_type InvalidateCache(size_t offset, size_t length) override {
                return target_->InvalidateCache(offset, length);
            }

            status_type RangeSync(uint64_t offset, uint64_t nbytes) override {
